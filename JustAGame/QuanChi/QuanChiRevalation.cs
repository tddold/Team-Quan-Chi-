...
namespace QuanChi
{
    class QuanChi
    {
        ...
        readonly static PictureFrame pictureFrame = new PictureFrame(Constants.Wall, Constants.PictureFrameWidth, Constants.PictureFrameHeight);
        static int lives = 3;
        static int score = 0;
        ...
        static void Results()
        {
            ...
            Console.BackgroundColor = ConsoleColor.Black;
            Console.Write("\t\t\tScore: {0,5}   Percentage: {1,2}%   Lives: {2}", score, percentage, lives);
        }
        private static void FillFieldDFS(List<BadBoys> badBoys)
        {
            ...
            int fullPercent = (pictureFrame.Hight) * (pictureFrame.Width);

            for (int row = 0; row < pictureFrame.Hight; row++)
            {
                for (int col = 0; col < pictureFrame.Width; col++)
                {
                    if (!visited[row, col])
                    {
                        countFilledCells++;
            ...
            score = 2 * countFilledCells - (pictureFrame.Hight * 2 + (pictureFrame.Width * 2));
            percentage = (countFilledCells - (pictureFrame.Hight * 2 + (pictureFrame.Width * 2))) * 100 / fullPercent;
            if (percentage < 80)
            {
                Results();
            }
